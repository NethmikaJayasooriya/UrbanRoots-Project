...
class GardenBasicsScreen extends StatefulWidget {
  // Callback forwarded from MyGardenScreen via GardenIntroScreen
  final Function(Map<String, dynamic>)? onGardenCreated;

  const GardenBasicsScreen({super.key, this.onGardenCreated});
...
class _GardenBasicsScreenState extends State<GardenBasicsScreen> {
  ...
  void _goToNextScreen() {
    ...
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) => IoTConnectionScreen(
          gardenData: gardenData,
          onGardenCreated: widget.onGardenCreated, // ← forward the callback
        ),
      ),
    );
  }
              ...
              Container(
                ...
                child: Row(
                  children: [
                    Icon(
                      Icons.location_on,
                      color: _latitude != null ? AppColors.primaryGreen : Colors.grey,
                      size: 20,
                    ),
                    ...
                    Expanded(
                      child: _isLocating
                          ? const LinearProgressIndicator(
                              color: AppColors.primaryGreen, minHeight: 2)
                          : Text(
                              _locationText,
                              style: GoogleFonts.poppins(
                                color: AppColors.textMain,
