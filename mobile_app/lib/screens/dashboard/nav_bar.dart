...
class _BioNavBarState extends State<_BioNavBar> with TickerProviderStateMixin {
  ...
  // Maps logical nav-item positions to slot indices (0–3 excluding FAB center)
  ...
  /// Returns 0–3 slot for the active item, -1 if none
  int get _activeSlot {
    ...
    return SizedBox(
      ...
      child: Stack(
        ...
        children: [
          ...
          // Orbit ring B — slow, dashed, opposite direction
          AnimatedBuilder(
            animation: _orbitB,
            builder: (_, __) => Transform.rotate(
...
// ─────────────────────────────────────────────────────────────────────────────
//  LIQUID BLOB INDICATOR  — slides smoothly between slots
// ─────────────────────────────────────────────────────────────────────────────
class _LiquidIndicator extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final double barW = MediaQuery.of(context).size.width - 32; // 2×sidePad
    // 4 items take the full width minus the 72px FAB gap
    final double itemW = (barW - 72) / 4;
    // Slot 0,1 are left of FAB; slot 2,3 are right of FAB
    double left;
    if (slot < 2) {
      left = slot * itemW;
    } else {
    ...
    return AnimatedPositioned(
      ...
      left: left + (itemW / 2) - 24, // 48px wide
      top: 10, // Moved up to catch the icon that translates -5px up
      child: Container(
        width: 48,
        height: 58, // Taller to cover the icon and dot perfectly
        decoration: BoxDecoration(
      ...
      child: SizedBox(
        ...
        child: Column(
          ...
          children: [
              ...
              child: AnimatedScale(
                ...
                child: ShaderMask(
                  shaderCallback: (bounds) => selected
                      ...
                      : const LinearGradient(
                          colors: [Color(0x88FFFFFF), Color(0x88FFFFFF)], // Increased from 44 to 88 for better visibility
                        ).createShader(bounds),
            ...
            AnimatedDefaultTextStyle(
              ...
              style: GoogleFonts.barlowCondensed(
                ...
                color: selected
                    ...
                    : Colors.white.withOpacity(0.55), // Increased from 0.22 for better readability
            ...
            // Glowing dot indicator (replaces underline)
            AnimatedContainer(
              duration: const Duration(milliseconds: 380),
...
// ─────────────────────────────────────────────────────────────────────────────
//  CUSTOM CLIPPER — shapes the bar with a soft center arch for FAB
// ─────────────────────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────────────────────
//  CUSTOM PAINTER — draws the bar background with shimmer & hex grid texture
// ─────────────────────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────────────────────
//  ORBIT RING PAINTER — dashed circular orbit for FAB
// ─────────────────────────────────────────────────────────────────────────────
