...
export class GardenService {
  ...
  // getGardenByUserId
  // ──────────────────────────────────────────────────────────────────────────────
  async getGardenByUserId(userId: string) {
    const garden = await this.gardenRepository.findOne({ where: { user_id: userId } });
    if (!garden) return null; // Return cleanly instead of throwing so frontend can handle normal onboarding
    return garden;
  }
